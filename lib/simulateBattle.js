...
function simulateBattle(pokeAtk, pokeDef, options) {
  if (pokeAtk.key) {
    mutateFromKey(pokeAtk)
  }
  if (pokeDef.key) {
    mutateFromKey(pokeDef)
  }
    ...
    if (winDef) {
      if (winDef.ko) {
        if (isRaid) {
          ...
          AttackerState = createState(pokeAtk, timeRemaining - 1000, AttackerState.totalDMG);
          log.push({
            p: 'atk',
            m: '@SWITCH',
            dmg: null,
